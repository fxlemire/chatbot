...
#WHOTO="@" # Remove the '#' and put a default e-mail account to talk to

myroster = {}  # my Buddy list
id2name = {}   # real names for buddies (if available)

# chatter.py is where the string handling is done
import chatter
...
class Talkbot206(sleekxmpp.ClientXMPP):
	...
	# "gg bot" is the status in the chat. Can be changed to anything
        self.send_presence(pstatus="gg bot", pshow='available', ppriority=12) 
    ...
    def message(self, msg):
        ...
        global replied, last, counterWhat, counterHow
        # If replied is not in the global variables, initialize replied to 0 and last to an empty string (so that it happens only once)
        ...
        if msg['type'] in ('chat', 'normal'):
            ...
            if self.autoreply:
                if replied<30:  # only reply a few times just in case of ugly bugs
                    ...
                    # prepare a reply
                    response = chatter.respond( text, counterWhat, counterHow, sender=msg['from'] )
		    # warn user that next message will be the last command
		    if replied==29:
			replytext = "%s\n\nSince I am a weak bot, I can only reply 30 times. This is now the 29th/30 reply. Next command is your last one. Choose it carefully!" % response
			print "Sending reply", replytext
			# send the reply
			msg.reply( replytext ).send()
		    # reply that it's the last message before the bot goes off
		    elif replied==30:
		        replytext = "%s\n\nThanks for discussing with me. This was my last reply. I will now hibernate. Have a great day!" % response
		        print "Sending reply", replytext
		        # send the reply
		        msg.reply( replytext ).send()
		    else:
                        # stock response: pretty boring, isn't it?
                        #replytext = "Thanks for sending: %s[...] [reply #%d]\n%s" % (text[0:9],replied,response)
...
if __name__ == '__main__':
    ...
    elif "facebook" in opts.jid.lower():
        # facebook addresses go to chat.facebook.com
        status = xmpp.connect(('chat.facebook.com', 5222))
    else: 
...
print "-"*60
print " - This is the talk206 chat client (part of Talk2013)."
